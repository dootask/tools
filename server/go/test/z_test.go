...
func TestGetBotList(t *testing.T) {
	client := setupTestClient()

	t.Run("获取机器人列表", func(t *testing.T) {
		botList, err := client.GetBotList()
		if err != nil {
			t.Fatalf("获取机器人列表失败: %v", err)
		}
		t.Logf("机器人列表: %s", formatJSON(botList.List))
	})
}
